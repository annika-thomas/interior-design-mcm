...
import { SURFACE_FOR_MATERIAL, surfaceMaterial } from './textures';
...
const MATERIAL_PROPS: Record<string, { roughness: number; metalness: number; repeat: number }> = {
  walnut: { roughness: 0.58, metalness: 0, repeat: 2 },
  teak: { roughness: 0.54, metalness: 0, repeat: 2 },
  oak: { roughness: 0.62, metalness: 0, repeat: 2.4 },
  rosewood: { roughness: 0.5, metalness: 0, repeat: 2 },
  bentply: { roughness: 0.46, metalness: 0, repeat: 1.6 },
  fiberglass: { roughness: 0.4, metalness: 0, repeat: 3 },
  'wool-boucle': { roughness: 0.94, metalness: 0, repeat: 4 },
  tweed: { roughness: 0.92, metalness: 0, repeat: 5 },
  leather: { roughness: 0.44, metalness: 0, repeat: 3 },
  brass: { roughness: 0.3, metalness: 0.85, repeat: 1.5 },
  'blackened-steel': { roughness: 0.4, metalness: 0.75, repeat: 1.5 },
  chrome: { roughness: 0.08, metalness: 0.95, repeat: 1.5 },
  travertine: { roughness: 0.38, metalness: 0, repeat: 2 },
  'smoked-glass': { roughness: 0.06, metalness: 0.2, repeat: 1 },
  rattan: { roughness: 0.78, metalness: 0, repeat: 3 },
  greywash: { roughness: 0.7, metalness: 0, repeat: 2.4 },
};

/**
 * A material for one part of a piece.
 *
 * Delegates to the procedural surface library so that a walnut credenza
 * actually has grain running across its doors and a tweed sofa has a weave you
 * can see from across the room. Materials are cached and shared, so nothing
 * here may be mutated by a caller.
 */
function mat(
  color: string,
  materialKey?: string,
  opts: { doubleSide?: boolean; emissive?: string; flat?: boolean } = {},
) {
  const props = MATERIAL_PROPS[materialKey ?? ''] ?? { roughness: 0.62, metalness: 0, repeat: 2 };
  const kind = SURFACE_FOR_MATERIAL[materialKey ?? ''] ?? 'plaster';
  return surfaceMaterial({
    ...
    // A glowing bulb or a flat graphic panel wants no relief at all.
    textureStrength: opts.flat || opts.emissive ? 0 : 1,
  });
}
...
interface BuildContext {
  ...
  /** Smooth surface for foliage, which is not made of whatever the pot is. */
  organic: THREE.Material;
...
export function buildPiece(piece: Furniture, item: CatalogItem | null): BuiltPiece {
  ...
  const ctx: BuildContext = {
    ...
    organic: mat(primaryColor, undefined, { doubleSide: true }),
